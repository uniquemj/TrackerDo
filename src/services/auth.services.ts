...
} from "../types/model/user.types.js";
...
export class AuthUserService {
  constructor(
    @inject(UserRepository) private readonly userRespository: UserRepository,
  ) {}

  async createUser(userParams: UserParams): Promise<string> {
    try {
      const userExist = await this.userRespository.findOneByEmail(
        userParams.email,
      );
      if (userExist) {
      ...
      const userPayload = {
        ...userParams,
        role: userParams.role ?? ROLE.USER,
        password: passwordHash,
      };
      const user = await this.userRespository.create(userPayload);
  ...
  async getMe(userParams: JWTUserPayload): Promise<UserProfileData> {
    try {
      const userProfile = await this.userRespository.findById(
        userParams.user_id,
      );
      if (!userProfile) {
