...
import type {
  ...
  WorkTimeParams,
} from "../types/model/worktime.types.js";
...
export class WorkTimeService {
  ...
  updateWorkTime = async (
    wt_id: string,
    user_id: string,
    dataLoad: UpdateWorkTimeParams,
  ) => {
    try {
      await this.getWorkTime(wt_id, user_id);
      const result = await this.workTimeRepository.updateWorkTime(
        wt_id,
        user_id,
        dataLoad,
      );
  ...
  deleteWorkTime = async (wt_id: string, user_id: string) => {
    try {
      await this.getWorkTime(wt_id, user_id);
      const result = await this.workTimeRepository.deleteWorkTime(
        wt_id,
        user_id,
      );
      return result;
