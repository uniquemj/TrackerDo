...
export class TodoService {
  ...
  getAllTask = async(queryParam: searchParams, paginationData?: PaginationData) =>{
    try{
  ...
  createTask = async (user_id: string, taskPayload: TodoParams) => {
    try {
      let { title, description, start_time, end_time, status, category_id } = taskPayload;
  ...
  updateTask = async(user_id: string, task_id: string, updateParams: UpdateTodoParams)=>{
    try{
